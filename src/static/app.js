      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (details.participants && details.participants.length > 0) {
          ...
          details.participants.forEach((email) => {
            participantsHTML += `<li><span class="participant-email">${email}</span><button class="delete-btn" data-activity="${name}" data-email="${email}" aria-label="Remove ${email}">✕</button></li>`;
          });
          participantsHTML += "</ul>";
        } else {
